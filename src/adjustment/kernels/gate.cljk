(ns adjustment.kernels.gate
  "Safety kernel for the loss-adjustment governor + phase gate — the
  decision CORE of `adjustment.governor/check` and
  `adjustment.phase/gate`, extracted into the safe-kotoba subset
  ...
  (`adjustment.governor`, `adjustment.phase`) reduce their inputs to
  ...
  Wire codes:
    ...
    op          0 read (this repo's read-ops set is EMPTY, so the façade
                ...
                1 :matter/intake            2 :jurisdiction/assess
                3 :conflict/screen          4 :valuation/finalize
  ...
  LESS autonomy (hold/escalate), never more. `:valuation/finalize`
  (op 4) is auto-enabled at NO phase — the same structural invariant
  the phase table and the governor's actuation gate state
  independently (finalizing a real valuation report is always a human
  independent adjuster's call)."
  )
...
(defn hard-violation
  ...
  spec-basis missing / conflict-of-interest hit / required supporting
  evidence incomplete."
  [spec-missing conflict-hit evidence-incomplete]
...
(defn verdict-code
  ...
  [spec-missing conflict-hit evidence-incomplete confidence-x100 actuation]
  (if (= 1 (hard-violation spec-missing conflict-hit evidence-incomplete))
    2
...
(defn op-auto-enabled
  ...
  column). Exactly one cell is ever 1: phase 3 x :matter/intake.
  op 4 (:valuation/finalize) is 0 at every phase — permanent
...
(defn phase-disposition
  ...
  governor's disposition code. Mirrors `adjustment.phase/gate`:
...
(defn check-verdict [spec conflict evid conf act expected]
  (if (= (verdict-code spec conflict evid conf act) expected) 1 0))
