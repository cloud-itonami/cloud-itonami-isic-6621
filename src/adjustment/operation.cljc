(ns adjustment.operation
  "OperationActor -- one loss-adjustment operation = one supervised actor
  run, expressed as a langgraph-clj StateGraph. The advisor
  (Adjuster-LLM) is sealed into a single node (:advise); its proposal is
  ALWAYS routed through the Loss Adjustment Governor (:govern) and the
  ...
  One graph run = one loss-adjustment operation (intake -> advise ->
  ...
  decision to a human operator (the licensed independent adjuster). The
  approver resumes with `{:approval {:status :approved}}` (or
  :rejected). `:valuation/finalize` ALWAYS reaches this node when the
  governor is clean -- see `adjustment.phase`."
...
(defn build
  ...
  opts:
    :advisor      -- an `adjustment.adjusterllm/Advisor` (default: mock-advisor)
    :checkpointer -- langgraph checkpointer (default: in-mem)"
  [store & [{:keys [advisor checkpointer]
             :or   {advisor      (adjusterllm/mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; Adjuster-LLM inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (adjusterllm/-advise advisor store request)]
            {:proposal p :audit [(adjusterllm/trace request p)]})))

      ;; Loss Adjustment Governor -- independent censor (separate system than the LLM).
      (g/add-node :govern
