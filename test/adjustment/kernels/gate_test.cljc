(ns adjustment.kernels.gate-test
  ...
  3. governor boundary — the confidence floor boundary and the
     ...
     through the real `adjustment.governor/check` façade."
...
(deftest valuation-finalize-auto-enabled-nowhere
  (testing "op 4 (:valuation/finalize) is auto-enabled at NO phase — kernel
            restates the phase table's permanent structural invariant"
    (doseq [phase [-1 0 1 2 3 4 7]]
      (is (= 0 (gate/op-auto-enabled phase 4))))))

;; ---------------------------------------------------------------
;; Governor boundary through the real façade. op :matter/intake
;; touches neither the store nor the evidence/spec checks, so the
;; verdict is decided purely by confidence/actuation — nil store is
;; safe.

(defn- verdict [proposal]
  (governor/check {:op :matter/intake :subject "matter-x"} {} proposal nil))
...
(deftest actuation-still-escalates-and-hard-still-wins
  ...
  (testing "a hard violation dominates actuation escalation"
    (let [v (governor/check {:op :jurisdiction/assess :subject "matter-x"} {}
